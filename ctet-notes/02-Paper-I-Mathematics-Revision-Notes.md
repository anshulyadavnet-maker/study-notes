...
### :icon-exam: Paper I Mathematics का structure (ढाँचा)
...
```figure
type: strategy-ctet
caption: Paper I Mathematics values children’s different strategies and reasoning
```
...
::: formula 📊 Split (विभाजन)
15 Content + 15 Pedagogy = 30 questions।
:::
...
::: formula 📌 Exam approach (परीक्षा दृष्टिकोण)
...
## 2. :icon-number: Numbers और Number Sense (संख्याएँ और संख्या-बोध)
...
```figure
type: place-value-ctet
number: 342
caption: hundreds, tens and ones show how place changes a digit’s value
```
...
caption: on a number line, moving right means increasing value
```
...
198 + 302 का estimate ≈ 200 + 300 = 500
```

---

## 3. :icon-calc: Operations (संक्रियाएँ) और Number Strategies
...
25 + 25 + 25 + 25 = 100
5 × 20 = 100
100 − 4 × 25 = 0  (यह expression context के अनुसार समझाएँ)
```
...
num: 3
den: 4
caption: numerator counts shaded parts and denominator counts equal parts
```

**Example:** 3/4 में whole को 4 equal parts में बाँटा गया और 3 parts selected हैं।
...
**Multiplication:** Numerator × numerator और denominator × denominator।
...
## 5. :icon-target: Geometry, Shapes और Spatial Understanding
...
```figure
type: shape-symmetry
caption: reflection lines help learners see symmetry in familiar shapes
```
...
```figure
type: solid-net-ctet
caption: six square faces can fold into a cube
```
...
## 6. :icon-ruler: Measurement, Weight, Time और Volume
...
caption: begin measurement at zero and read the scale carefully
```
...
hour: 3
minute: 30
caption: minute hand at 6 shows 30 minutes past the hour
```
...
- Area की unit square होती है: cm², m²।
...
## 7. :icon-clock: Money, Patterns और Data Handling
...
```figure
type: money-ctet
caption: combine notes and coins, then check the total amount
```
...
```figure
type: pattern-ctet
caption: identify the repeated change before predicting the next term
```
...
```figure
type: math-data-ctet
caption: bar height represents the value in each category
```
...
## 8. :icon-brain: Nature of Mathematics और Pedagogy
...
```figure
type: community-math
caption: daily contexts connect money, time, measure and shape with mathematics
```

---

## 9. :icon-warn: Errors, Misconceptions और Remedial Teaching
...
caption: analyse the child’s strategy before planning mathematical support
```
...
## 10. :icon-check: Evaluation और Assessment in Mathematics

```figure
type: assessment-cycle
caption: teach, observe, diagnose and adapt using mathematical evidence
```
...
::: remember 🎯 Golden teacher response
...
2. 17 prime क्यों है?
3. 3/4 और 5/8 compare करने का एक तरीका लिखिए।
4. 2/3 + 1/6 solve कीजिए।
...
::: formula 🎯 Final revision rule (अंतिम पुनरावृत्ति नियम)
